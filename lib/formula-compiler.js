(function (global, factory) {
    ...
    function compile(exp, mode, namespace, useContext) {
        ...
        if (namespace === undefined) namespace = "workbook.fn";
            ...
            switch (node.type) {
                case "operator":
                    switch (node.subtype) {
                        ...
                        case "infix-subtract":
                            return namespace + ".MINUS(" + compiler(node.operands[0]) + "," + compiler(node.operands[1]) + ")";
                        case "infix-multiply":
                ...
                case "function":
                    switch (node.name) {
                        ...
                        case "AND":
                            return namespace + ".AND(" + node.args.map(function (n) {
                                return compiler(n);
                            }).join(", ") + ")";
                        case "OR":
                            return namespace + ".OR(" + node.args.map(function (n) {
                                return compiler(n);
                            }).join(", ") + ")";
                ...
                case "value":
                    switch (node.subtype) {
                        case "array":
                            return "[" + node.items.map(function (n) {
                                return compiler(n);
                            }).join(",") + "]";
                        case "string":
